...
5. If the user also needs a PDF, hand the generated HTML to `$report-pdf-print` instead of rebuilding layout logic in a second place.
...
2. Invoke `$report-pdf-print` on that HTML file
...
3. If PDF output is needed, call `$report-pdf-print` on the generated HTML
