...
If a repository already has a report config for `$html-report`, reuse the same file here.
...
If the PDF looks wrong because sections overflow or cards are too dense, fix the HTML source in `$html-report`.

Use `$pdf-print` only to control:
- paper size
- orientation
- margins
- footer label
