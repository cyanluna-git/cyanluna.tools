...
If the task started from markdown, prefer `$html-report` first and then run `$pdf-print` on the generated HTML.
...
- `$html-report` owns report layout and branding
- `$pdf-print` owns final PDF printing

Do not move report composition logic into `$pdf-print`.
